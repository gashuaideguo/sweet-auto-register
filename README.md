...
当前管理页只监听本机地址 `127.0.0.1`，用于本地管理服务：
- 启动自动化流程
- 触发 CPA 文件同步
- 查看注册账号数、CPA 文件数、当前手机号状态、错误截图数
- 查看内存中的脱敏日志
- 编辑项目根目录 `config.yaml`
...
配置页面会通过表单写入项目根目录 `config.yaml`，保存前会做基础校验，避免写入明显错误的端口、URL 或必填同步配置。

敏感字段（例如 API Key、密码）默认不会回显；如果输入框留空后保存，会保留原来的值；只有填写新值时才会覆盖。

第一版限制：
- 不展示原始密钥、账号文件内容或 token 内容
- 状态页只显示数量和配置是否已填写
- 同一时间只允许运行一个任务
